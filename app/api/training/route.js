...
export const GET = async req => {
  ...
  const { data: training, error } = await supabase.from('training').select()
  if (error) {
    "Couldn't get training. Look at /@services/api/trainingApi.js.  ERROR:", error
  }
  // console.log('server api: ', training)
...
export const DELETE = async (req, res) => {
  ...
  const { data, error } = await supabase.from('training').delete().eq('id', id)
...
//     return NextResponse.json(training);
//   } catch (error) {
//     console.error("Couldn't get training. Look at /@services/api/trainingApi.js.  ERROR:", error);
...
//     switch (type) {
//       case 'toggleCompletion':
//         const { error } = await supabase.from('training').update({ completed: updateStatus }).eq('id', id);
//         if (error) throw error;
...
//     const { data, error } = await supabase.from('training').insert([{ id, title, completed }]);
//     if (error) throw error;
